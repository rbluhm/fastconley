                          ...
                          real colvector time, real matrix S, real scalar pixel)
{
	...
	if (all(grp)) {                 // nothing to merge; no panel metadata needed
...
{
	real scalar n, k, ang, edge, G, coscut, b, nb, lo, hi, ncell, ca, cb, o
	real scalar cx, cy, cz, nx, ny, nz, u, npairs, direct
	real colvector latr, lonr, key, p, ucid, lookup
	real matrix U, M, tinfo, cinfo, offs, cell, neighbors
	...
	// Bound the direct-address map at 64 MiB. Small cutoffs use a sparse
	// numeric neighbor table instead; no live asarray may reach the worker
	// loop (Mata function calls become very costly while it is in scope).
	direct = (G^3 <= 64 * 1024^2 / 8)
	if (direct) lookup = J(G^3, 1, 0)
	...
	for (b = 1; b <= nb; b++) {
		...
		ncell = rows(cinfo)
		ucid = key[cinfo[., 1]]
		if (direct) lookup[ucid :+ 1] = (1::ncell)
		else neighbors = reghdfe_conley_cell_neighbors(ucid, G, offs)
		for (ca = 1; ca <= ncell; ca++) {
			...
			for (o = 1; o <= 13; o++) {
				...
				u = (nx * G + ny) * G + nz
				if (direct) cb = lookup[u + 1]
				else cb = neighbors[ca, o]
				if (!cb) continue
				npairs = npairs + reghdfe_conley_cell_pair(M, U, S, T, k, cinfo[ca, 1], cinfo[ca, 2],
		...
		// Clear only occupied entries before the next period's cell map.
		if (direct) lookup[ucid :+ 1] = J(ncell, 1, 0)
	}
...
// Build the sparse map in its own scope and release the dictionary before
// returning. Columns retain the original 13-offset order; zero means absent.
real matrix reghdfe_conley_cell_neighbors(real colvector ucid, real scalar G,
                                       real matrix offs)
{
	transmorphic A
	real matrix neighbors
	real scalar c, o, u, cx, cy, cz, nx, ny, nz, ncell
	ncell = rows(ucid)
	neighbors = J(ncell, 13, 0)
	A = asarray_create("real", 1)
	for (c = 1; c <= ncell; c++) asarray(A, ucid[c], c)
	for (c = 1; c <= ncell; c++) {
		u = ucid[c]
		cz = mod(u, G); cy = mod(floor(u / G), G); cx = floor(u / (G * G))
		for (o = 1; o <= 13; o++) {
			nx = cx + offs[o, 1]; ny = cy + offs[o, 2]; nz = cz + offs[o, 3]
			if (nx < 0 | nx >= G | ny < 0 | ny >= G | nz < 0 | nz >= G) continue
			u = (nx * G + ny) * G + nz
			if (asarray_contains(A, u)) neighbors[c, o] = asarray(A, u)
		}
	}
	A = J(0, 0, .)
	return(neighbors)
...
real scalar reghdfe_conley_detect_grid(real colvector lat, real colvector lon, real scalar tol)
{
	...
	ul = uniqrows(lat)
	if (rows(ul) < 2) return(0)
	dl = ul[|2 \ rows(ul)|] - ul[|1 \ rows(ul)-1|]
	step_l = min(dl)
	if (step_l <= 0) return(0)
	...
	// Scattered latitudes already rule out a raster: sort longitude only
	// after latitude has passed its lattice test.
	uo = uniqrows(lon)
	if (rows(uo) < 2) return(0)
	dol = uo[|2 \ rows(uo)|] - uo[|1 \ rows(uo)-1|]
	step_o = min(dol)
	if (step_o <= 0) return(0)
...
//   fc_n_periods    period count, also used by the ado and Mata meat step
// Inputs:
                             ...
                             real scalar balanced, real scalar pixel, real scalar verbose)
{
	...
	external real scalar fc_sp_balanced, fc_n_periods

	n = rows(fc_S)
	// A constant time column (including omitted time()) needs no unique sort.
	// Aggregation never removes a period, so this count applies to fc_sp_* too.
	T = 1
	if (n > 1) {
		if (!all(time :== time[1])) T = rows(uniqrows(time))
	}
	fc_n_periods = T
                                 ...
                                 real scalar tile, real scalar verbose)
{
	...
	external real scalar fc_n_periods
	external real colvector fc_sp_time
	T = 1
	plat = lat; plon = lon; ptime = time; Sstack = S
	if (balanced) {
		// Preparation supplies the count for its own time vector. Keep the
		// standalone matrix API valid for callers supplying other rows too.
		if ((&time) == (&fc_sp_time)) T = fc_n_periods
		else T = rows(uniqrows(time))
	}
	if (T > 1) {
		n_per = rows(S) / T
		kk = cols(S)
